...
const SignUp = (props) => {
  ...
  const handleChange = (event) => {
    ...
    setValuesregi({ ...valuesregi, [name]: value });
  };

  const handleSubmit = (event) => {
    event.preventDefault();
    let errorCreate = validateregi(valuesregi);
    setErrorCreate(errorCreate);
      ...
      signUPApi(payload)
        .then((res) => {
          if (res?.status === 200) {
            ...
            // props.navigate("/"); // Redirect to the desired page upon successful signup
          navigate("/"); 
          }
        })
        .catch((err) => {
                ...
                <div className="input-container">
                  <div className="right-inner-addon input-container">
                    ...
                    <i className="fa fa-search">
                      <AiOutlineLock
                        size={30}
                        style={{ color: "black", paddingBottom: "3px" }}
                      />
                    </i>
                  </div>
                  <input
                    className="signup-input-box passworerror"
                    onChange={handleChange}
